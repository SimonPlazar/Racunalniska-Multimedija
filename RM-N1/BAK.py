...
def kompresija(vhodni_niz, tabela_verjetnosti, kumulativna_frekvenca, stevilo_bitov=8):
    ...
    # Kodiranje posameznega znaka
    for znak in vhodni_niz:
        # Dobimo verjetnostni interval za znak
        ...
        # Posodobimo meje glede na interval znaka
    ...
    if spodnja_meja < prva_cetrtina:
        izhod.append('01')
        izhod.extend(['1'] * e3_counter)
    else:
        izhod.append('10')
        izhod.extend(['0'] * e3_counter)

    return ''.join(izhod)


def dekompresija(encoded_bits, tabela_verjetnosti, kumulativna_frekvenca, stevilo_bitov=8):
    # Inicializacija globalnega intervala
    spodnja_meja = 0
    zgornja_meja = (2 ** (stevilo_bitov-1)) - 1
    ...
    output = []
    while 1:
    # while trenutni_bit_index < len(encoded_bits):
        # Izračun koraka
        korak = (zgornja_meja - spodnja_meja + 1) // kumulativna_frekvenca
        ...
        if len(output) == kumulativna_frekvenca:
            break
...
def compress_file(vhodna_datoteka, izhodna_datoteka, stevilo_bitov):
    # Ustvarimo slovar za štetje frekvenc znakov
    frekvence = defaultdict(int)

    # vhodni_niz = bytearray()
    vhodni_niz = ''
    # Preberemo datoteko znak po znak
    with open(vhodna_datoteka, 'rb') as vhodna:
        while (znak := vhodna.read(1)):
            frekvence[znak.decode()] += 1
            vhodni_niz += znak.decode()
    ...
    # Izpišemo tabelo verjetnosti
    print("Tabela verjetnosti:")
    print(f"{'Znak':<4} {'Frekvenca':<10} {'spMeja':<8} {'zgMeja':<8}")
    for znak, (sp_meja, zg_meja) in tabela_verjetnosti.items():
        print(
            f"{znak.decode() if isinstance(znak, bytes) else znak:<4} {frekvence[znak]:<10} {sp_meja:<8} {zg_meja:<8}")

    output = kompresija(vhodni_niz, tabela_verjetnosti, kumulativna_frekvenca, stevilo_bitov=stevilo_bitov)
    # print(f"Kodirani niz: {output}")
    # print(f"Vhodni niz: {vhodni_niz}")

    stevilo_bytov = stevilo_bitov // 8

    # Na koncu lahko shranite izhod v datoteko
    with open(izhodna_datoteka, 'wb') as izhodna:
        # zakodiramo tabelo verjetnosti in kumulativno frekvenco
        izhodna.write(stevilo_bitov.to_bytes(1))
        izhodna.write(len(tabela_verjetnosti).to_bytes(stevilo_bytov))
        # izhodna.write(kumulativna_frekvenca.to_bytes(1))
        for znak, (sp_meja, zg_meja) in tabela_verjetnosti.items():
            izhodna.write(znak.encode())
            # izhodna.write(sp_meja.to_bytes(1))
            # izhodna.write(zg_meja.to_bytes(1))
            izhodna.write(frekvence[znak].to_bytes(stevilo_bytov))

        izhodna.write(output.encode())


def decompress_file(vhodna_datoteka, izhodna_datoteka):
    with open(vhodna_datoteka, 'rb') as vhodna:
        stevilo_bitov = int.from_bytes(vhodna.read(1))
        stevilo_bytov = stevilo_bitov // 8
        # Preberemo število znakov
        st_znakov = int.from_bytes(vhodna.read(stevilo_bytov))

        kumulativna_frekvenca = 0

        tabela_verjetnosti = {}
        for _ in range(st_znakov):
            znak = vhodna.read(1).decode()  # Preberemo en znak
            # sp_meja = int.from_bytes(vhodna.read(1))
            # zg_meja = int.from_bytes(vhodna.read(1))
            # tabela_verjetnosti[znak] = (sp_meja, zg_meja)
            frekvenca = int.from_bytes(vhodna.read(stevilo_bytov))
            tabela_verjetnosti[znak] = (kumulativna_frekvenca, kumulativna_frekvenca + frekvenca)
            kumulativna_frekvenca += frekvenca

        vhodni_biti = vhodna.read().decode()

        # Izpišemo tabelo verjetnosti
        # print("Prebrana tabela verjetnosti:")
        # print(f"{'Znak':<4} {'spMeja':<8} {'zgMeja':<8}")
        # for znak, (sp_meja, zg_meja) in tabela_verjetnosti.items():
        #     print(f"{znak if isinstance(znak, bytes) else znak:<4} {sp_meja:<8} {zg_meja:<8}")
        # print(f"Kumulativna frekvenca: {kumulativna_frekvenca}")
        # print(f"Število bitov: {stevilo_bitov}")

        output = dekompresija(vhodni_biti, tabela_verjetnosti, kumulativna_frekvenca, stevilo_bitov=stevilo_bitov)
        # print(f"Dekodirani niz: {output}")
